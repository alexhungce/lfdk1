	...
	{                                                                      \
		...
		if (copy_from_user(&DATA, argp, sizeof(DATA))) {               \
			...
			return -EFAULT;                                        \
                                                                               ...
                                                                               \
		DATA.buf = RFUNC(lfdd_cal_pci_addr(DATA.bus, DATA.dev,         \
						   DATA.fun, DATA.reg));       \
                                                                               \
	...
	{                                                                      \
		...
		if (copy_from_user(&DATA, argp, sizeof(DATA))) {               \
			...
			return -EFAULT;                                        \
                                                                               ...
                                                                               \
		WFUNC(DATA.buf, lfdd_cal_pci_addr(DATA.bus, DATA.dev,          \
						  DATA.fun, DATA.reg));        \
		return 0;                                                      \
	}
...
static int lfdd_open(struct inode *inode, struct file *file) { return 0; }

static int lfdd_release(struct inode *inode, struct file *file) { return 0; }
...
static long lfdd_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LFDD_PCI_READ_256BYTE:
		...
		printk(KERN_INFO "C3\n");
		if (copy_from_user(&lfdd_pci_data, argp,
				   sizeof(struct lfdd_pci_t))) {
		...
		return copy_to_user(argp, &lfdd_pci_data,
				    sizeof(struct lfdd_pci_t));
	...
	case LFDD_MEM_READ_256BYTE:
		...
		if (copy_from_user(&lfdd_mem_data, argp,
				   sizeof(struct lfdd_mem_t))) {
		...
		return copy_to_user(argp, &lfdd_mem_data,
				    sizeof(struct lfdd_mem_t));
	...
	case LFDD_IO_READ_256BYTE:
		...
		if (copy_from_user(&lfdd_io_data, argp,
				   sizeof(struct lfdd_io_t))) {
		...
		return copy_to_user(argp, &lfdd_io_data,
				    sizeof(struct lfdd_io_t));
