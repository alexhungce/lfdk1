	...
	for (bus = 0; bus < maxpcibus; bus++) {
		...
		for (dev = 0; dev <= 0x1f; dev++) {
			...
			for (fun = 0; fun <= 0x07; fun++) {
				...
				PrintFixedWin(
				    PCILScreen, scan, 1, 36, 10, 25, WHITE_BLUE,
				    "Bus = %2.2X, Dev = %2.2X, Fun = %2.2X",
				    bus, dev, fun);
				update_panels();
				doupdate();
...
void PrintPCIScreen(int fd)
{
	...
	if (lfdd_pci_list[curr_index].ventxt[0]) {
		...
		PrintFixedWin(PCIScreen, ven, 1, 70, 1, 1, CYAN_BLUE,
			      "Vendor: %.62s",
			      lfdd_pci_list[curr_index].ventxt);
	} else {
	...
	if (lfdd_pci_list[curr_index].devtxt[0]) {
		...
		PrintFixedWin(PCIScreen, dev, 1, 70, 2, 1, CYAN_BLUE,
			      "Device: %.62s",
			      lfdd_pci_list[curr_index].devtxt);
	} else {
	...
	wprintw(PCIScreen.rtitle, "VID:DID = %4.4X:%4.4X\n",
		lfdd_pci_list[curr_index].venid,
		lfdd_pci_list[curr_index].devid);
		...
		for (j = 0; j < LFDK_BYTE_PER_LINE; j++) {
			...
			if (y == j && x == i) {
				...
				if (input) {
					...
					wattrset(PCIScreen.value,
						 COLOR_PAIR(BLACK_YELLOW) |
						     A_BOLD);
				}
			} else if (((unsigned char)lfdd_pci_data
			...
			if (y == j && x == i) {
				...
				if (input) {
					...
					wprintw(PCIScreen.value, "%2.2X",
						(unsigned char)wbuf);
				} else {

					wprintw(
					    PCIScreen.value, "%2.2X",
					    (unsigned char)lfdd_pci_data
						.mass_buf[(i *
							   LFDK_BYTE_PER_LINE) +
							  j]);
				}
...
void PrintPCILScreen(void)
{
	...
	for (i = curr_index; ((i < (curr_index + rpp)) && (i < last_index));
	     i++) {

		if ((i - curr_index) == lightbar) {

			wattrset(PCILScreen.devname,
		...
		if (!lfdd_pci_list[i].ventxt[0]) {
			...
			wprintw(PCILScreen.devname, "%4.4X, ",
				lfdd_pci_list[i].venid);
		} else {

			wprintw(PCILScreen.devname, "%.12s, ",
				lfdd_pci_list[i].ventxt);
		}
	...
	for (i = curr_index; ((i < (curr_index + rpp)) && (i < last_index));
		...
		wprintw(PCILScreen.vendev,
			"%4.4X    %4.4X     %2.2X   %2.2X   %2.2X\n",
			lfdd_pci_list[i].venid, lfdd_pci_list[i].devid,
			lfdd_pci_list[i].bus, lfdd_pci_list[i].dev,
			lfdd_pci_list[i].fun);
	}
