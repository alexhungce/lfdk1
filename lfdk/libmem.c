...
void PrintMemScreen(int fd)
{
	...
	for (i = 0; i < LFDK_BYTE_PER_LINE; i++) {
		...
		for (j = 0; j < LFDK_BYTE_PER_LINE; j++) {
			...
			tmp = ((unsigned char)lfdd_mem_data
		...
		for (j = 0; j < LFDK_BYTE_PER_LINE; j++) {
			...
			if (y == j && x == i) {
				...
				if (input) {
					...
					wattrset(MemScreen.value,
						 COLOR_PAIR(BLACK_YELLOW) |
						     A_BOLD);
				}
			} else if (((unsigned char)lfdd_mem_data
			...
			if (y == j && x == i) {
				...
				if (input) {
					...
					wprintw(MemScreen.value, "%2.2X",
						(unsigned char)wbuf);
				} else {

					wprintw(
					    MemScreen.value, "%2.2X",
					    (unsigned char)lfdd_mem_data
						.mass_buf[(i *
							   LFDK_BYTE_PER_LINE) +
							  j]);
				}
